						...
						<>
							<div className="flex-shrink-0">
								<div className="avatar-sm">
									<span className={classNames('avatar-title', 'bg-' + project.variant + '-lighten', 'text-' + project.variant, 'rounded')}>
										<i className={classNames(project.icon, 'font-24')} />
									</span>
								</div>
							</div>
							<div className="flex-grow-1 ms-3">
								<Link href="" className="font-16 fw-bold text-secondary">
									{project.title}
					...
					<Col xs={6} className="text-end">
						...
						<div className="multi-user">
							{(modifiedTeamMembers || []).map((assign, index) => {
								return (
									<Link href="" className="d-inline-block" key={index.toString()}>
										<Image src={assign.avatar} alt="" className="rounded-circle avatar-xs" />
									</Link>
								);
							})}
							{project.assignTo.length > modifiedTeamMembers.length && (
								<Link href="" className="d-inline-block ms-n2">
									<div className="avatar-xs">
										<span className="avatar-title bg-primary rounded-circle">{project.assignTo.length - displayCount}+</span>
									</div>
								</Link>
							)}
						</div>

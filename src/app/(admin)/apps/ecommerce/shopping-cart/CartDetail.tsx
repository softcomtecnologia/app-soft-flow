	...
	return (
		<Row>
			<Col lg={8}>
						...
						{items.map((item, index) => {
							return (
								<tr key={index.toString()}>
									<td>
										<Image src={item.image} alt={item.name} title="contact-img" className="rounded me-3" height="64" />
				...
				<Row className="mt-4">
					<Col sm={6}>
						<Link href="/apps/ecommerce/products" className="btn text-muted d-none d-sm-inline-block btn-link fw-semibold">
							<i className="mdi mdi-arrow-left"></i> Continue Shopping
						</Link>
					</Col>
					<Col sm={6}>
						<div className="text-sm-end">
							<Link href="/apps/ecommerce/checkout" className="btn btn-danger">

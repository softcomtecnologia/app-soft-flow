...
import { Comment, Person } from '@/app/(admin)/apps/social/types';
...
const FormPost = () => {
	return (
		<form action="#" className="comment-area-box">
			<textarea rows={3} className="form-control border-0 resize-none" placeholder="Write something...."></textarea>
...
const PostComment = ({ comment }: PostCommentProps) => {
	return (
		<div className="d-flex">
			{comment.author.avatar && <Image className="me-2 rounded-circle" src={comment.author.avatar} alt="" height="32" />}
			<div>
				<h5 className="mt-0">
				...
				{comment.replies ? (
					<Link href="" className="text-muted font-13 d-inline-block mt-2 mb-3">
...
const Post = ({ post, user }: PostProps) => {
	return (
		<div className="border border-light rounded p-2 mb-3">
			<div className="d-flex">
				{post.author.avatar && <Image className="me-2 rounded-circle" src={post.author.avatar} alt="" height="32" />}
				<div>
					...
					{user && (
						<div className="d-flex mt-2">
							<Link href="" className="pe-2">
								{user.avatar && <Image src={user.avatar} height="32" className="rounded-circle" alt="" />}
							</Link>
							<div className="w-100">
								<input type="text" className="form-control border-0 form-control-sm" placeholder="Add comment" />
							</div>
						</div>
					)}
				</div>
